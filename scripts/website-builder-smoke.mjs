...
check('Keeping a partially reviewed AI result requires explicit confirmation', builder.includes('function approveAICandidatePreview()') && builder.includes('unreviewedCount > 0') && builder.includes("l('affected pages have not been reviewed. Keep result anyway?')") && builder.includes('onClick={approveAICandidatePreview}'));
...
check('Candidate target reveal selects the valid comparison side for additions and removals', builder.includes("if (operation.kind === 'remove') setAICandidatePreviewMode('before')") && builder.includes("if (operation.kind === 'add') setAICandidatePreviewMode('after')") && builder.includes('window.requestAnimationFrame(() => {\n      window.requestAnimationFrame(() => {'));
