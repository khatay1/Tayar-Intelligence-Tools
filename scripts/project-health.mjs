...
check('Website Builder AI edit prompt returns targeted patch operations', aiPrompts.includes("action === 'edit'") && aiPrompts.includes('update_section|add_section|remove_section|update_page|restyle_site|update_site') && aiPrompts.includes('Do not return a full "pages" replacement in edit mode'));
...
check('Website Builder AI applies generated palette to the full theme', websiteBuilder.includes('generatedSurfaceIsLight') && websiteBuilder.includes('generatedMutedTextColor') && websiteBuilder.includes('setTheme((current) => normalizeTheme') && websiteBuilder.includes('setHeaderConfig((current) =>'));
