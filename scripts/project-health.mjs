...
check('Retired template sync Edge Function remains present as a 410 guard', exists('supabase/functions/template-library-sync/index.ts'));
check('Retired template discovery Edge Function remains present as a 410 guard', exists('supabase/functions/template-library-discover/index.ts'));
check('Retired template Drive discovery Edge Function remains present as a 410 guard', exists('supabase/functions/template-library-drive-discover/index.ts'));
check('Retired template browser importer stays removed', !exists('scripts/template-library-browser-importer.js'));
check('Template R2 architecture documentation exists', exists('docs/TEMPLATE_LIBRARY_MIRROR.md'));
check('R2 template migration runner exists', exists('scripts/template-library-r2-migrate.mjs'));
check('Supabase template purge runner exists', exists('scripts/template-library-supabase-purge.mjs'));
...
const templateDiscoverSource = read('supabase/functions/template-library-discover/index.ts');
const templateSyncSource = read('supabase/functions/template-library-sync/index.ts');
const templateArchitecture = read('docs/TEMPLATE_LIBRARY_MIRROR.md');
...
check('Legacy Drive template discovery is retired with HTTP 410', templateDriveDiscoverSource.includes('status: 410') && templateDriveDiscoverSource.includes('retired') && templateDriveDiscoverSource.includes('Cloudflare R2') && !templateDriveDiscoverSource.includes('fetch('));
check('Legacy template discovery is retired with HTTP 410', templateDiscoverSource.includes('status: 410') && templateDiscoverSource.includes('retired') && templateDiscoverSource.includes('Cloudflare R2') && !templateDiscoverSource.includes('fetch('));
check('Legacy template sync is retired with HTTP 410', templateSyncSource.includes('status: 410') && templateSyncSource.includes('retired') && templateSyncSource.includes('Cloudflare R2') && !templateSyncSource.includes('.storage.from('));
...
check('Template architecture documents R2 delivery and retired Supabase import', templateArchitecture.includes('Cloudflare R2') && templateArchitecture.includes('legacy Supabase template import pipeline has been retired') && templateArchitecture.includes('Do not restore browser-driven bulk importers'));
...
check('Template metadata migration preserves catalog and import history schema', templateMirrorMigration.includes("'template-library'") && templateMirrorMigration.includes('template_assets') && templateMirrorMigration.includes('template_import_runs'));
check('Retired template sync endpoint cannot restart Supabase Storage imports', templateMirrorSync.includes('status: 410') && templateMirrorSync.includes('retired') && templateMirrorSync.includes('Cloudflare R2') && !templateMirrorSync.includes('.storage.from('));
check('Retired template discovery endpoint cannot crawl remote sources', templateMirrorDiscover.includes('status: 410') && templateMirrorDiscover.includes('retired') && templateMirrorDiscover.includes('Cloudflare R2') && !templateMirrorDiscover.includes('fetch('));
...
check('Background Remover is registered as local browser AI', modulesIndex.includes("import './background-remover'") && read('src/modules/background-remover/index.ts').includes("status: 'beta'") && backgroundRemoverTool.includes('Local processing') && backgroundRemoverTool.includes('No image is sent to fal.ai, Gemini or Tayar servers.'));
check('Background Remover client validates MIME and file size before local inference', backgroundRemoverClient.includes('BACKGROUND_ALLOWED_TYPES.has(file.type)') && backgroundRemoverClient.includes('MAX_BACKGROUND_IMAGE_BYTES') && backgroundRemoverClient.includes('validateBackgroundFile(file)'));
...
check('Website Builder supports direct canvas element resizing', websiteBuilder.includes('renderSelectedElementResizeHandle') && websiteBuilder.includes('cursor-ew-resize') && websiteBuilder.includes('onResizeElementFrame'));
check('Canvas resize is device-aware and undo-friendly', websiteBuilder.includes('beginElementResize') && websiteBuilder.includes("remember(sections, 'Resize element')") && websiteBuilder.includes('[device]:') && websiteBuilder.includes('width: safeWidth'));
...
check('Selected element toolbar follows canvas move offsets', websiteBuilder.includes('transform: `translate3d(${positionX}px, ${positionY}px, 0)`') && websiteBuilder.includes('Drag with smart element and center guides'));
