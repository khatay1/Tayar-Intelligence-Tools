...
check('Website Builder AI edit prompt returns targeted patch operations', aiPrompts.includes("action === 'edit'") && aiPrompts.includes('add_page|remove_page|set_home_page|update_section|add_section|remove_section|update_page|restyle_site|update_site|update_seo|update_header|generate_image') && aiPrompts.includes('Do not return a full "pages" replacement in edit mode'));
...
check('AI editable snapshot includes SEO header and element context', websiteBuilder.includes('seo: {') && websiteBuilder.includes('header: {') && websiteBuilder.includes('responsiveModes: Object.keys(element.responsive || {})'));
...
check('AI patch engine safely removes only non-home pages', websiteBuilder.includes("operation.action === 'remove_page'") && websiteBuilder.includes('nextPages.length <= 1') && websiteBuilder.includes('targetPage.id === homePageId') && websiteBuilder.includes('nextPages.splice(pageIndex, 1)'));
