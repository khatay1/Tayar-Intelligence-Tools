...
export default function CsvCleanerTool({ darkMode: _darkMode }: { darkMode: boolean }) {
  ...
  const previewColumns = Math.min(12, previewRows.reduce((max, row) => Math.max(max, row.length), 0));
  ...
  return (
    <ToolShell icon={FileSpreadsheet} title={l('CSV Cleaner')} description={l('Clean and prepare CSV data safely without uploading it.')} badge="Local processing">
      ...
      <div className="grid min-w-0 xl:grid-cols-[360px_minmax(0,1fr)] gap-4 sm:gap-6 items-start">
        <ToolInputPanel>
          <label className="block rounded-xl border border-dashed border-white/15 bg-white/[0.025] hover:bg-white/[0.05] p-4 sm:p-5 text-center cursor-pointer transition-colors">
            ...
            <input type="file" accept=".csv,.tsv,.txt,text/csv,text/tab-separated-values,text/plain" onChange={handleFileChange} className="hidden" />
          </label>

          {document && sourceStats && <>
            <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3 text-xs text-gray-400 min-w-0">
              <div className="text-white font-medium truncate">{document.name}</div>
              <div className="mt-1 break-words">{sourceStats.rows.toLocaleString()} {l('rows')} · {sourceStats.columns} {l('columns')} · {formatBytes(document.size)}</div>
            ...
            <div className="space-y-2">
              {[
                ...
                <label key={key} className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.025] px-3 py-2.5 cursor-pointer">
                  <span className="text-sm text-gray-300 min-w-0 break-words">{l(label)}</span>
                  <input type="checkbox" checked={options[key as keyof CsvCleanOptions]} onChange={() => toggleOption(key as keyof CsvCleanOptions)} className="accent-violet-500 shrink-0" />
                </label>
              ))}
            </div>
            <button type="button" onClick={() => void cleanData()} disabled={loading} className="w-full min-h-11 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-60 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors">
              <Sparkles className="w-4 h-4" />{l(loading ? 'Cleaning...' : 'Clean CSV')}
            </button>
            {cleanedRows && resultStats && <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-3 text-xs text-gray-300"><div className="font-medium text-violet-200">{l('Cleaned result')}</div><div className="mt-1">{resultStats.rows.toLocaleString()} {l('rows')} · {resultStats.columns} {l('columns')}</div><div className="mt-1 text-gray-500">{Math.max(0, sourceStats.rows - resultStats.rows).toLocaleString()} {l('rows removed')}</div></div>}
            {cleanedRows && <button type="button" onClick={downloadCleaned} className="w-full min-h-11 flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white text-sm font-medium px-4 py-2.5 transition-colors"><Download className="w-4 h-4" />{l('Download Clean CSV')}</button>}
          </>}
        ...
        <ToolOutputPanel loading={loading && !document} hasContent={Boolean(document)} empty={<div className="py-16 text-center text-sm text-gray-600">{l('Choose a file to preview its data.')}</div>}>
          {document && <div className="min-w-0">
            <div className="flex items-center justify-between gap-3 mb-3"><div className="min-w-0"><div className="text-white text-sm font-semibold">{l(cleanedRows ? 'Cleaned preview' : 'Source preview')}</div><div className="text-xs text-gray-500">{l('Showing up to 25 rows and 12 columns')}</div></div></div>
            <div className="overflow-auto rounded-xl border border-white/10 max-h-[620px] max-w-full">
              <table className="min-w-max text-xs"><tbody>{previewRows.map((row, rowIndex) => <tr key={rowIndex} className={rowIndex === 0 ? 'bg-white/[0.07]' : 'border-t border-white/5'}>{Array.from({ length: previewColumns }, (_, columnIndex) => <td key={columnIndex} className="px-3 py-2.5 min-w-[120px] max-w-[260px] text-gray-300 truncate" title={row[columnIndex] || ''}>{row[columnIndex] || ''}</td>)}</tr>)}</tbody></table>
            </div>
            {previewRows.length === 0 && <div className="py-12 text-center text-sm text-gray-600">{l('No rows remain after cleaning.')}</div>}
          </div>}
        </ToolOutputPanel>
      </div>
    </ToolShell>
