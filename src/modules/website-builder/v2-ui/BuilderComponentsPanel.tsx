...
export function BuilderComponentsPanel({
  ...
  const metrics = useMemo(() => {
    const totalInstances = symbols.reduce((sum, symbol) => sum + (instanceCounts[symbol.id] || 0), 0);
    const variantGroups = new Set(symbols.map((symbol) => symbol.variantGroupId).filter(Boolean)).size;
    const unused = symbols.filter((symbol) => !(instanceCounts[symbol.id] || 0)).length;
    return { totalInstances, variantGroups, unused };
  ...
  const createVariant = (symbol: EditorSymbolLike) => {
    ...
    const name = window.prompt(l('Variant name'), l('Variant'))?.trim();
    if (!name) return;
    onCreateVariant(symbol.id, name.slice(0, 64));
  };

  const openMetadata = (symbol: EditorSymbolLike) => {
    setEditingId(symbol.id);
    setDraft(metadataDraft(symbol));
  };

  const saveMetadata = () => {
    if (!editingId || !onUpdateMetadata || disabled) return;
    onUpdateMetadata(editingId, {
  ...
  return (
    <div className="tayar-v2-components-panel" aria-busy={disabled}>
      ...
      {!!symbols.length && (
        <div className="grid grid-cols-3 gap-1.5 text-center text-[9px] text-gray-400">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] px-1.5 py-2"><strong className="block text-xs text-white">{metrics.variantGroups}</strong>{l('variant groups')}</div>
          <div className="rounded-lg border border-white/10 bg-white/[0.03] px-1.5 py-2"><strong className="block text-xs text-white">{metrics.unused}</strong>{l('unused')}</div>
        </div>
      )}
      ...
      <div className="tayar-v2-component-list">
        {visibleSymbols.map((symbol) => {
          ...
          return (
            <div className="tayar-v2-component-row flex-wrap" key={symbol.id} data-active={activeSymbolId === symbol.id ? 'true' : 'false'}>
              <button
                type="button"
              ...
              >
                ...
                <span className="tayar-v2-component-row__meta">
                  ...
                  <small>{count} {l('instances')}{symbol.variantName ? ` · ${symbol.variantName}` : ''}{symbol.category ? ` · ${symbol.category}` : ''}</small>
                  {Array.isArray(symbol.tags) && symbol.tags.length > 0 && <small>{symbol.tags.slice(0, 4).join(' · ')}</small>}
                </span>
              </button>
              ...
              <button type="button" disabled={disabled || !onCreateVariant} onClick={() => createVariant(symbol)} title={l('Create variant')}>{l('VAR')}</button>
              ...
              <button type="button" disabled={disabled || !onUpdateMetadata} onClick={() => isEditing ? setEditingId(null) : openMetadata(symbol)} title={l('Component details')}>{l('INFO')}</button>
              ...
              {isEditing && (
                <div className="w-full space-y-2 rounded-lg border border-white/10 bg-black/10 p-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <label className="block text-[9px] text-gray-400">{l('Category')}<input className="mt-1 w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-[10px] text-white" maxLength={48} value={draft.category} onChange={(event) => setDraft((current) => ({ ...current, category: event.target.value }))} /></label>
                    <label className="block text-[9px] text-gray-400">{l('Variant name')}<input className="mt-1 w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-[10px] text-white" maxLength={64} value={draft.variantName} onChange={(event) => setDraft((current) => ({ ...current, variantName: event.target.value }))} disabled={!symbol.variantGroupId} /></label>
                  </div>
                  <label className="block text-[9px] text-gray-400">{l('Tags')}<input className="mt-1 w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-[10px] text-white" value={draft.tags} onChange={(event) => setDraft((current) => ({ ...current, tags: event.target.value }))} placeholder={l('Comma-separated tags')} /></label>
                  <div className="flex justify-end gap-2"><button type="button" onClick={() => setEditingId(null)}>{l('Cancel')}</button><button type="button" onClick={saveMetadata} disabled={disabled || !onUpdateMetadata}>{l('Save details')}</button></div>
                </div>
              )}
            </div>
          );
