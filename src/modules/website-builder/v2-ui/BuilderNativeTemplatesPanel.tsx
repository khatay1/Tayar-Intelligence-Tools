...
export function BuilderNativeTemplatesPanel({ disabled = false, onInsert, onPreview, onCustomizeWithAI }: BuilderNativeTemplatesPanelProps) {
  ...
  return <section className="space-y-3 border-b border-white/10 p-3" aria-label={l('Website templates')}>
    <div><div className="text-sm font-semibold text-white">{l('Website templates')}</div><p className="mt-1 text-[11px] leading-4 text-gray-500">{l('Sections, pages and complete sites ready to insert.')}</p></div>
    <input type="search" value={query} onChange={event => setQuery(event.currentTarget.value)} placeholder={l('Search templates')} aria-label={l('Search templates')} className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-white outline-none" />
    ...
    <div className="space-y-2">{items.map(item => <article key={item.id} className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
      <div className="flex items-center justify-between gap-2"><strong className="text-xs text-gray-100">{l(item.name)}</strong><span className="rounded bg-white/5 px-1.5 py-1 text-[9px] uppercase text-gray-500">{l(item.kind === 'section' ? 'Sections' : item.kind === 'page' ? 'Pages' : 'Sites')}</span></div>
