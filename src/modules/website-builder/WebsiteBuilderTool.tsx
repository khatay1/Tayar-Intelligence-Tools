...
const [seo, setSeo] = useState<WebsiteSEO>(defaultSEO);
  ...
  function applyProjectData(parsed: any, loadHistory = true) {
    ...
    if (parsed?.sections && Array.isArray(parsed.sections) && parsed.sections.length) {
      ...
      setPages([{ id: 'page-home', name: 'Home', slug: 'home', sections: normalized, showInNavigation: true, language: normalizePageLanguage(parsed?.language, 'en'), translationKey: 'home' }]);
...
if (generated.seo) {
  ...
  function exportAnalyticsCsv() {
    const rows = analyticsEvents.map((event) => ({
      ...
      event_data: event.event_data ? JSON.stringify(event.event_data) : '',
    }));
    downloadCsv(`${normalizeSlug(siteName || 'website')}-analytics.csv`, rows);
  }
