...
export function loadLocalWebsiteProject(): unknown | null {
  ...
  for (const key of LOCAL_PROJECT_KEYS) {
    ...
    hydrateEditorMaxProjectState(project);
    return project;
  }
...
export function saveLocalWebsiteProject(project: unknown): boolean {
  ...
  try {
    storage.setItem(STORAGE_KEY, JSON.stringify(embedEditorMaxProjectState(project)));
    return true;
  } catch {
    return false;
  }
  ...
  try {
    storage.setItem(
      ...
      JSON.stringify({
        ...
        project: embedEditorMaxProjectState(project),
      }),
    );
    return true;
  } catch {
...
export function loadRecoveryWebsiteProject<TProject = unknown>(): EditorRecoverySnapshot<TProject> | null {
  ...
  hydrateEditorMaxProjectState(parsed.project);
  return {
