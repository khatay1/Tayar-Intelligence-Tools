...
import { ImageOutputFormat, ImageSourceInfo, OUTPUT_FORMATS } from './image-types';
import { inspectImage, processImage, safeOutputName } from './image-processing';
...
export default function ImageToolsTool({ darkMode: _darkMode }: { darkMode: boolean }) {
  ...
  function downloadResult() {
    ...
    const anchor = document.createElement('a'); anchor.href = outputUrl; anchor.download = safeOutputName(source.name, selectedFormat.extension); anchor.rel = 'noopener'; anchor.click();
  }
  ...
  return (
    <ToolShell icon={ImageIcon} title={l('Image Tools')} description={l('Resize, compress and convert images directly in your browser.')} badge="Local processing">
      ...
      <div className="grid min-w-0 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)] gap-4 sm:gap-6 items-start">
        <ToolInputPanel>
          <ToolField label={l('Source image')}><label className="block rounded-xl border border-dashed border-white/15 bg-white/[0.025] hover:bg-white/[0.05] p-4 sm:p-5 text-center cursor-pointer transition-colors"><ImageIcon className="w-7 h-7 text-violet-400 mx-auto mb-2" /><div className="text-sm text-white font-medium">{l('Choose JPEG, PNG or WebP')}</div><div className="text-xs text-gray-500 mt-1">{l('Maximum 20 MB · bounded pixel processing')}</div><input type="file" accept="image/jpeg,image/png,image/webp" onChange={handleFileChange} className="hidden" /></label></ToolField>
          {source && <>
        ...
        <ToolOutputPanel loading={processing} hasContent={Boolean(source)} empty={<div className="py-16 text-center text-sm text-gray-600">{l('Choose an image to start.')}</div>}>
          {source && <div className="space-y-4 min-w-0"><div className="grid md:grid-cols-2 gap-4"><div><div className="text-xs uppercase tracking-wider text-gray-500 mb-2">{l('Original')}</div><div className="rounded-xl overflow-hidden bg-black/20 border border-white/10 min-h-[180px] sm:min-h-[220px] flex items-center justify-center">{previewUrl && <img src={previewUrl} alt={l('Original preview')} className="max-h-[420px] max-w-full object-contain" />}</div></div><div><div className="text-xs uppercase tracking-wider text-gray-500 mb-2">{l('Result')}</div><div className="rounded-xl overflow-hidden bg-black/20 border border-white/10 min-h-[180px] sm:min-h-[220px] flex items-center justify-center">{outputUrl ? <img src={outputUrl} alt={l('Processed preview')} className="max-h-[420px] max-w-full object-contain" /> : <div className="text-xs text-gray-600 text-center px-6">{l('Process the image to preview the result.')}</div>}</div></div></div>
            {outputBlob && <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div className="text-sm min-w-0"><div className="text-white font-medium break-words">{selectedFormat.label} · {width} × {height}px</div><div className="text-xs text-gray-500 mt-0.5 break-words">{formatBytes(outputBlob.size)}{source.size > 0 && ` · ${Math.round((1 - outputBlob.size / source.size) * 100)}% size change`}</div></div><button type="button" onClick={downloadResult} className="min-h-11 inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold px-4 py-2.5 transition-colors"><Download className="w-4 h-4" />{l('Download')}</button></div>}
          </div>}
        </ToolOutputPanel>
      </div>
    </ToolShell>
