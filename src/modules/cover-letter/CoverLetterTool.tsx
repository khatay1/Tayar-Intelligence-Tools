...
export default function CoverLetterTool({ darkMode: _darkMode }: { darkMode: boolean }) {
  ...
  async function handleGenerate() {
    ...
    setGenerating(true);
    setResult('');
    const toastId = loading('Writing cover letter...');
    try {
      ...
      update(toastId, 'Cover letter written', 'success');
    } catch (err) {
  ...
  return (
    <ToolShell icon={Mail} title="Cover Letter Writer" description="Craft personalized cover letters for any job." badge="v2.0">
      <div className="grid lg:grid-cols-2 gap-6">
        <ToolInputPanel>
          <ToolField label="Your Name">
          ...
          <ToolField label="Company">
            <input value={company} onChange={e => setCompany(e.target.value)} className={toolInputClass} placeholder="Google" />
          </ToolField>
          <ToolField label="Key Qualifications">
            <textarea value={qualifications} onChange={e => setQualifications(e.target.value)} className={`${toolInputClass} min-h-[80px] resize-y`} placeholder="5 years React, led team of 4..." />
          </ToolField>
          <ToolField label="Why this job?">
            <textarea value={motivation} onChange={e => setMotivation(e.target.value)} className={`${toolInputClass} min-h-[60px] resize-y`} placeholder="Passionate about..." />
          </ToolField>
