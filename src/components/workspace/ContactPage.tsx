...
export default function ContactPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from('notifications').insert({
        title: `Contact: ${cleanSubject || 'New message'}`,
        message: `From ${cleanName} (${cleanEmail}): ${cleanMessage}`,
        type: 'contact',
      });

      if (error) throw error;

      toast.update(toastId, 'Message sent successfully! We\'ll get back to you soon.', 'success');
      setName('');
      setEmail('');
      setSubject('');
      setMessage('');
      ...
      toast.update(toastId, 'Failed to send message. Please try again.', 'error');
    }
    setSending(false);
  }
  ...
  return (
    <PageShell icon={Mail} title="Contact Us" subtitle="We'd love to hear from you. Reach out with any questions or feedback.">
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <div><div className="text-white text-sm font-medium">Email</div><div className="text-gray-500 text-xs">support@tayar.ai</div></div>
        </div>
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <div><div className="text-white text-sm font-medium">Phone</div><div className="text-gray-500 text-xs">+46 8 123 45 67</div></div>
        </div>
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <div><div className="text-white text-sm font-medium">Office</div><div className="text-gray-500 text-xs">Stockholm, Sweden</div></div>
        </div>
      </div>
