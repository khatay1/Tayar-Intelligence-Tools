      ...
      <div className="relative bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl p-6 sm:p-8 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">{l('Welcome back, {name}').replace('{name}', displayName)}</h1>
            <p className="text-gray-400 text-sm">{l('{all} tools available · {ready} ready to use').replace('{all}', String(allTools.length)).replace('{ready}', String(availableTools.length))}</p>
          </div>
          <button
      ...
      <button onClick={onClick} className="flex items-center gap-3 flex-1 min-w-0 text-left">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-gray-500 text-xs">
            {cat?.label}
            {showUsage && usageCount > 0 && ` · ${l('Used {count}x').replace('{count}', String(usageCount))}`}
          </div>
        </div>
      </button>
      <button
