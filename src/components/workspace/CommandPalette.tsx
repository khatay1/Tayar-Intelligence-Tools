    ...
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-start justify-center pt-[12vh] px-4" onClick={onClose}>
      ...
      >
        ...
        <div className="max-h-[55vh] overflow-y-auto p-2">
          {grouped.length === 0 && !loading ? (
            <div className="py-10 text-center">
              ...
              <p className="text-gray-500 text-sm">{l('No results for')} "{query}"</p>
            </div>
          ) : (
            grouped.map(group => (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-white text-sm font-medium truncate">{l(item.label)}</div>
                        {item.subtitle && <div className="text-gray-500 text-xs truncate">{l(item.subtitle)}</div>}
                      </div>
        ...
        <div className="px-4 py-2.5 border-t border-white/5 flex items-center justify-between text-xs text-gray-600">
          ...
          <span>{results.length} {l('results')}</span>
        </div>
      </div>
    </div>
  );
