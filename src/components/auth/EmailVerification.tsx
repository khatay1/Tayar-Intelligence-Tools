...
export default function EmailVerification({ onBack, onNavigate }: EmailVerificationProps) {
  ...
  async function handleResend() {
    if (!email) {
      setError(l('Enter your email first.'));
      return;
    }
    setError(null);
    setLoading(true);
  ...
  return (
    <AuthLayout onBack={onBack}>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-gray-300 text-sm font-medium mb-1.5">{l("Didn't get it? Enter your email")}</label>
        ...
        >
          ...
          {l(loading ? 'Sending...' : resent ? 'Email sent!' : 'Resend Verification Email')}
        </button>
