...
const COPY = {
  ...
  ar: {
    ...
    source: 'لقطة بيانات الدليل', refreshed: 'محدّثة', getKey: 'الحصول على API key', apiKey: 'مفتاح API', keepSecret: 'اتركه فارغًا للاحتفاظ بالمفتاح الحالي', accountId: 'Cloudflare Account ID', model: 'النموذج المقترح', configure: 'إعداد المزود', update: 'تحديث المزود', test: 'اختبار', makeDefault: 'جعله الافتراضي', configured: 'مُعدّ', missingSecret: 'مفتاح API مطلوب', enabled: 'مفعّل', disabled: 'معطّل', default: 'افتراضي', permanent: 'خطة مجانية دائمة', renewable: 'حصة مجانية/متجددة', noCard: 'بدون بطاقة حسب الدليل', registration: 'يتطلب تسجيل', phone: 'يتطلب تحقق هاتف', loading: 'جارٍ تحميل حالة المزودين...', loadError: 'تعذر تحميل المزودين المدارين.', saveError: 'تعذر حفظ المزود.', saved: 'تم إعداد المزود وحفظ المفتاح بأمان.', activated: 'تم تغيير مزود AI الافتراضي.', tested: 'نجح اتصال المزود.', accountHelp: 'تجد هذا المعرّف المكوّن من 32 خانة في لوحة Cloudflare. يبني Tayar منه عنوان Workers AI الرسمي المتوافق مع OpenAI.', safety: 'لا يتم تفعيل أي شيء تلقائيًا قبل أن تضبط المزود بمفتاحك. التوجيه الحالي والـfallback الإنتاجي يبقيان كما هما إلى أن تغيّرهما أنت صراحة.',
  },
  sv: {
...
export default function AdminFreeLLMCatalog() {
  ...
  async function configure(provider: FreeLLMProviderPreset) {
    ...
    const { error } = await supabase.functions.invoke('ai-admin-control', {
      body: {
        action: 'save', providerKey: provider.key, label: provider.label, adapter: provider.adapter,
        baseUrl, model, apiSecret: apiSecret || undefined, enabled: true,
      },
    });
    setBusy(null);
  ...
  async function testProvider(providerKey: string) {
    ...
    setBusy(null);
    if (error) { showError(error.message || c.loadError); return; }
    success(`${c.tested}${data?.latencyMs ? ` · ${data.latencyMs} ms` : ''}`);
  }
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.035] p-4 sm:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-300">{c.eyebrow}</div>
          <div className="mt-1 flex items-center gap-2"><Server className="h-5 w-5 text-emerald-300" /><h2 className="text-xl font-bold text-white">{c.title}</h2></div>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-gray-400">{c.description}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-gray-500">
            <span>{c.source} · {c.refreshed} {FREE_LLM_CATALOG_UPDATED_AT}</span>
            <a href={FREE_LLM_CATALOG_SOURCE} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-emerald-300 hover:text-emerald-200">source <ExternalLink className="h-3 w-3" /></a>
          </div>
        </div>
        <button onClick={() => void load()} disabled={loading} className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-4 text-sm text-gray-300 hover:bg-white/[0.06] disabled:opacity-60"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Refresh</button>
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {FREE_LLM_PROVIDER_PRESETS.map(provider => {
          const current = managed(provider.key);
          ...
          return <article key={provider.key} className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.025] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-white">{provider.label}</h3>{current && <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300">{c.configured}</span>}{current?.is_default && <span className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2 py-0.5 text-[10px] font-semibold text-cyan-300">{c.default}</span>}</div>
                <p className="mt-1 text-xs leading-5 text-gray-400">{provider.description}</p>
              </div>
              <div className="flex shrink-0 flex-wrap gap-1.5 text-[10px]"><span className="rounded-full border border-white/10 px-2 py-1 text-gray-300">{provider.freeTier === 'permanent' ? c.permanent : c.renewable}</span><span className="rounded-full border border-white/10 px-2 py-1 text-gray-400">{verificationLabel(provider, c)}</span></div>
            </div>

            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="min-w-0"><span className="mb-1.5 block text-[10px] font-bold uppercase tracking-wide text-gray-500">{c.model}</span><select value={selectedModel} onChange={event => setModels(previous => ({ ...previous, [provider.key]: event.target.value }))} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#0d0d1a] px-3 text-sm text-white">{provider.models.map(model => <option key={model.id} value={model.id}>{model.label}</option>)}</select></label>
              {provider.baseUrlNeedsAccountId ? <label className="min-w-0"><span className="mb-1.5 block text-[10px] font-bold uppercase tracking-wide text-gray-500">{c.accountId}</span><input value={accountIds[provider.key] || ''} onChange={event => setAccountIds(previous => ({ ...previous, [provider.key]: event.target.value }))} placeholder="0123456789abcdef0123456789abcdef" className="min-h-11 w-full rounded-xl border border-white/10 bg-[#0d0d1a] px-3 text-sm text-white outline-none focus:border-emerald-500/40" /></label> : <div className="min-w-0 rounded-xl border border-white/10 bg-black/10 px-3 py-2"><div className="text-[10px] font-bold uppercase tracking-wide text-gray-500">Base URL</div><div className="mt-1 truncate text-xs text-gray-300" title={provider.baseUrl}>{provider.baseUrl}</div></div>}
            </div>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
              <label className="min-w-0"><span className="mb-1.5 flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-wide text-gray-500"><span>{c.apiKey}</span><a href={provider.apiKeyUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 normal-case text-emerald-300 hover:text-emerald-200">{c.getKey}<ExternalLink className="h-3 w-3" /></a></span><div className="relative"><KeyRound className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-gray-600" /><input type="password" value={secrets[provider.key] || ''} onChange={event => setSecrets(previous => ({ ...previous, [provider.key]: event.target.value }))} placeholder={current?.secret_configured ? c.keepSecret : c.apiKey} autoComplete="new-password" className="min-h-11 w-full rounded-xl border border-white/10 bg-[#0d0d1a] pl-9 pr-3 text-sm text-white outline-none focus:border-emerald-500/40" /></div></label>
              <button onClick={() => void configure(provider)} disabled={Boolean(busy)} className="mt-auto inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-semibold text-white hover:bg-emerald-500 disabled:opacity-50">{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{current ? c.update : c.configure}</button>
            </div>

            {current && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-white/5 pt-3"><span className={`rounded-full border px-2 py-1 text-[10px] ${current.enabled ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300' : 'border-gray-500/20 bg-gray-500/10 text-gray-400'}`}>{current.enabled ? c.enabled : c.disabled}</span><span className={`rounded-full border px-2 py-1 text-[10px] ${current.secret_configured ? 'border-blue-500/20 bg-blue-500/10 text-blue-300' : 'border-amber-500/20 bg-amber-500/10 text-amber-300'}`}>{current.secret_configured ? c.apiKey : c.missingSecret}</span><button onClick={() => void testProvider(provider.key)} disabled={Boolean(busy) || !current.enabled || !current.secret_configured} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-white/10 px-3 text-xs text-gray-300 hover:bg-white/5 disabled:opacity-40">{busy === `test:${provider.key}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <TestTube2 className="h-3.5 w-3.5" />}{c.test}</button>{!current.is_default && <button onClick={() => void activateProvider(provider.key)} disabled={Boolean(busy) || !current.enabled || !current.secret_configured} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-cyan-500/20 px-3 text-xs text-cyan-300 hover:bg-cyan-500/10 disabled:opacity-40">{busy === `activate:${provider.key}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Server className="h-3.5 w-3.5" />}{c.makeDefault}</button>}</div>}
          </article>;
        })}
      </div>
    </section>
