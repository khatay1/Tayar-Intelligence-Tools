...
export default function AdminAI() {
  ...
  const load = useCallback(async () => {
    ...
    for (const u of usage) { const key = new Date(u.created_at).toISOString().split('T')[0]; if (days[key]) { days[key].requests++; days[key].tokens += u.tokens_in + u.tokens_out; } }
  ...
  return <div className="space-y-5 max-w-7xl mx-auto min-w-0 overflow-x-hidden">
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">{[
      { label: '14d Requests', value: totalRequests.toLocaleString(), icon: Zap, color: 'violet' }, { label: 'Total Tokens', value: totalTokens.toLocaleString(), icon: Activity, color: 'fuchsia' }, { label: 'Backend Providers', value: providers.filter(p => p.status === 'active' && p.service === 'gemini').length, icon: Server, color: 'emerald' }, { label: 'Recent Error Rate', value: `${errorRate.toFixed(1)}%`, icon: AlertCircle, color: 'amber' },
    ...
    <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-4 sm:p-5 min-w-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">{providers.map(p=>{const backendEnabled=p.service==='gemini'; const isActive=backendEnabled&&p.service===activeProvider; return <div key={p.id} className={`rounded-xl border p-4 min-w-0 ${isActive?'bg-violet-600/10 border-violet-500/30':'bg-white/[0.02] border-white/10'}`}><div className="flex items-start justify-between gap-2 mb-3"><div className="flex items-center gap-2.5 min-w-0"><div className={`w-9 h-9 rounded-xl shrink-0 flex items-center justify-center ${p.status==='active'?'bg-emerald-500/10':'bg-gray-500/10'}`}><Cpu className="w-4 h-4"/></div><div className="min-w-0"><div className="text-sm font-medium text-white truncate">{p.label}</div><div className="text-xs text-gray-500 capitalize truncate">{p.service}</div></div></div>{isActive&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-300 shrink-0">{l('Default')}</span>}</div><div className="flex flex-wrap items-center gap-2 mb-3"><span className={`w-2 h-2 rounded-full ${p.status==='active'?'bg-emerald-400':'bg-gray-600'}`}/><span className="text-xs text-gray-400">{!backendEnabled?'Backend not enabled':p.status==='active'?'Active':'Inactive'}</span>{p.last_used&&<span className="text-xs text-gray-600 sm:ml-auto">{new Date(p.last_used).toLocaleDateString()}</span>}</div><button onClick={()=>void switchProvider(p.service)} disabled={switching||isActive||!backendEnabled} className="w-full min-h-11 py-2 rounded-lg text-xs font-medium bg-white/5 text-gray-300 disabled:opacity-50">{!backendEnabled?'Backend not enabled':isActive?'Current Provider':'Set as Default'}</button></div>})}</div>
    </div>
