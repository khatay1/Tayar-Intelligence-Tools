...
export function useCVBuilderIntegration({ supabase, projects, userId, enabled = true }: Options) {
  ...
  const saveCurrentVersion = useCallback(() => versionState.save(core.cv, core.template), [versionState.save, core.cv, core.template]);
  ...
  return {
    ...
    quality,
    proposals,
    versions: { versions: versionState.versions, loading: versionState.loading, load: versionState.load, saveCurrent: saveCurrentVersion, restore: restoreVersion },
    jobDescription,
    setJobDescription,
    atsScore,
    setAtsScore,
