  ...
  async function handleAIAction(action: AIAction) {
    ...
    try {
      if (action === 'suggest-skills') {
        ...
        const exp = builder.cv.experience[0];
        if (!exp) throw new Error('Add experience first');
        ...
        addTextProposal({ kind: 'text-rewrite', section: 'experience', itemId: exp.id, before: exp.description, proposed: res.content.trim(), reason: 'AI rewrite. Verify every detail before applying.' });
      }
    } catch (error) {
