...
export default function ResumeBuilder({ onBack }: ResumeBuilderProps) {
  ...
  async function handleAIAction(action: AIAction) {
    ...
    try {
      if (action === 'suggest-skills') {
        ...
        addTextProposal({ kind: 'text-rewrite', section: 'experience', itemId: builder.cv.experience[0].id, before: builder.cv.experience[0].description, proposed: res.content.trim(), reason: 'AI rewrite. Verify every detail before applying.' });
      } else if (action === 'write-summary') {
        const res = await ai.complete({ action, jobTitle: builder.cv.personal.jobTitle, skills: builder.cv.skills.map(s => s.name), experiences: builder.cv.experience });
        addTextProposal({ kind: 'text-rewrite', section: 'summary', before: builder.cv.summary, proposed: res.content.trim(), reason: 'AI summary draft. Verify every statement before applying.' });
      ...
      toast.error(error instanceof AIError ? error.message : error instanceof Error ? error.message : 'AI request failed');
  ...
  async function doExport(format: 'pdf' | 'docx' | 'txt') {
    ...
    try {
      if (format === 'pdf') await exportToPDF('cv-preview', `${builder.cv.personal.fullName || 'resume'}.pdf`);
      else if (format === 'docx') await exportToDOCX(builder.cv, builder.template, builder.colorTheme, prefs.language);
  ...
  if (phase === 'template-select') return <div className="min-h-full p-6"><div className="max-w-5xl mx-auto"><button type="button" onClick={onBack} className="text-gray-400 hover:text-white text-sm mb-6 flex items-center gap-2"><ArrowLeft className="w-4 h-4" />{l('Back to Tools')}</button><div className="text-center mb-8"><h1 className="text-3xl font-bold text-white">{l('Choose a Template')}</h1><p className="text-gray-400 mt-2">Select a starting design. You can change it later.</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{TEMPLATES.map(template => <button type="button" key={template.id} onClick={() => chooseTemplate(template.id)} className="text-left bg-white/[0.03] border border-white/10 hover:border-violet-500/40 rounded-2xl p-5 transition"><div className="h-40 rounded-xl bg-white mb-4 flex items-center justify-center text-gray-800 font-bold">{template.name}</div><h3 className="text-white font-semibold">{template.name}</h3><p className="text-gray-500 text-xs mt-1">{template.description}</p></button>)}</div></div></div>;

  return <div className="h-full flex flex-col bg-[#09090f]">
    <header className="h-14 border-b border-white/10 flex items-center px-4 gap-3 flex-shrink-0"><button type="button" onClick={onBack} className="text-gray-400 hover:text-white"><ArrowLeft className="w-5 h-5" /></button><div className="text-white font-semibold flex-1">{builder.cv.personal.fullName || 'Untitled Resume'}</div><span className="text-[10px] text-gray-500">{builder.saveStatus}</span><button type="button" disabled={!builder.canUndo} onClick={builder.undo}><Undo2 className="w-4 h-4" /></button><button type="button" disabled={!builder.canRedo} onClick={builder.redo}><Redo2 className="w-4 h-4" /></button><button type="button" onClick={() => setZoom(v => Math.max(.5, v - .1))}><ZoomOut className="w-4 h-4" /></button><span className="text-xs text-gray-400">{Math.round(zoom * 100)}%</span><button type="button" onClick={() => setZoom(v => Math.min(1.5, v + .1))}><ZoomIn className="w-4 h-4" /></button><div className="relative"><button type="button" onClick={() => setExportMenu(v => !v)} className="bg-violet-600 text-white text-xs px-3 py-2 rounded-lg flex gap-1.5"><Download className="w-3.5 h-3.5" /> Export <ChevronDown className="w-3 h-3" /></button>{exportMenu && <div className="absolute right-0 mt-2 w-40 rounded-xl border border-white/10 bg-[#171722] shadow-xl z-50">{(['pdf','docx','txt'] as const).map(f => <button type="button" key={f} disabled={exporting} onClick={() => void doExport(f)} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/5">Export {f.toUpperCase()}</button>)}</div>}</div></header>
    <div className="flex-1 flex min-h-0"><aside className="w-16 border-r border-white/10 flex flex-col items-center py-3 gap-2">{panels.map(p => { const Icon=p.icon; return <button type="button" title={p.label} aria-label={p.label} aria-pressed={activePanel===p.id} key={p.id} onClick={() => setActivePanel(p.id)} className={`w-10 h-10 rounded-xl flex items-center justify-center ${activePanel===p.id?'bg-violet-600 text-white':'text-gray-500 hover:bg-white/5'}`}><Icon className="w-4 h-4" /></button>; })}</aside><aside className="w-[390px] border-r border-white/10 overflow-y-auto p-4"><CVBuilderShell panel={activePanel} cv={builder.cv} editor={editor} editSection={editSection} setEditSection={setEditSection} sections={editSections} template={builder.template} colorTheme={builder.colorTheme} fontId={builder.fontId} design={builder.sectionsController} setTemplate={builder.setTemplate} setColorTheme={builder.setColorTheme} setFontId={builder.setFontId} quality={builder.quality} jobDescription={builder.jobDescription} setJobDescription={builder.setJobDescription} versions={builder.versions.versions} versionsLoading={builder.versions.loading} saveVersion={builder.versions.saveCurrent} restoreVersion={builder.versions.restore} proposals={builder.proposals.proposals} applyProposal={builder.proposals.verifyAndApply} dismissProposal={builder.proposals.dismiss} setCV={builder.setData} />{activePanel==='ai' && <div className="mt-5 border-t border-white/10 pt-4 space-y-2"><h3 className="text-white text-xs font-semibold">AI Actions</h3>{AI_ACTIONS.map(a => <button type="button" disabled={!!aiLoading} onClick={() => void handleAIAction(a.id)} key={a.id} className="w-full text-left border border-white/10 rounded-lg p-2.5 text-xs text-gray-300 hover:border-violet-500/30 disabled:opacity-50"><div className="flex items-center gap-2"><Wand2 className="w-3.5 h-3.5 text-violet-400" />{a.label}{aiLoading===a.id && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}</div></button>)}</div>}</aside><main className="flex-1 min-w-0 overflow-auto p-8 bg-black/20"><div className="mx-auto origin-top transition-transform" style={{width:794, transform:`scale(${zoom})`}}><CVPreview cv={builder.cv} template={builder.template} colorTheme={builder.colorTheme} sections={builder.sections} fontId={builder.fontId} /></div><div className="max-w-3xl mx-auto mt-8"><AIScorePanel score={score} suggestions={suggestions} /></div></main></div>
    {showCoverLetter && coverLetter && <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"><div className="bg-[#171722] border border-white/10 rounded-2xl max-w-2xl w-full max-h-[80vh] flex flex-col"><div className="p-4 border-b border-white/10 flex items-center"><Mail className="w-4 h-4 text-violet-400 mr-2" /><span className="text-white font-semibold">Cover Letter</span><button type="button" className="ml-auto" onClick={() => setShowCoverLetter(false)}><X className="w-4 h-4" /></button></div><div className="p-5 overflow-y-auto whitespace-pre-wrap text-sm text-gray-300">{coverLetter}</div><div className="p-4 border-t border-white/10 flex gap-2"><input value={coverLetterCompany} onChange={e=>setCoverLetterCompany(e.target.value)} placeholder="Company" className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 text-sm text-white" /><button type="button" onClick={() => navigator.clipboard.writeText(coverLetter)} className="px-3 py-2 border border-white/10 rounded-lg text-xs text-gray-300 flex gap-1"><Copy className="w-3.5 h-3.5" />Copy</button><button type="button" onClick={() => void handleAIAction('cover-letter')} className="px-3 py-2 bg-violet-600 rounded-lg text-xs text-white">Regenerate</button></div></div></div>}
  </div>;
}
