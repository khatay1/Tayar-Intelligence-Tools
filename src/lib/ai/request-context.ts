...
export function setActiveAIProjectContext(tool: string, projectId: string | null, fingerprint: string | null = null): void {
  activeBindings.set(tool, { tool, projectId, fingerprint });
}

export function prepareAIProjectRequestContext(tool: string, projectId: string | null, fingerprint: string | null = null): void {
...
export function bindAIResponseProjectContext(value: unknown, binding: AIProjectRequestBinding | null): void {
...
export function assertAIResponseProjectContextCurrent(value: unknown, tool: string): void {
  ...
  if (binding.projectId !== null && active.projectId === binding.projectId && binding.fingerprint && active.fingerprint && binding.fingerprint !== active.fingerprint) {
    throw new Error('Target project files changed while AI was working. Generate a fresh patch before applying changes.');
  }
}

export function assertAIResponseProjectContextMatches(value: unknown, tool: string, projectId: string, fingerprint?: string | null): void {
  const target = objectValue(value);
