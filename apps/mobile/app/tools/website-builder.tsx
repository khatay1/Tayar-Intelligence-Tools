...
export default function WebsiteBuilderScreen() {
  ...
  async function saveProject() {
    ...
    try {
      ...
      setDirty(false);
      setMessage('Saved to Tayar Cloud.');
      void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (caught) {
      setError(errorText(caught));
  ...
  async function runAI() {
    if (!draft || !brief.trim() || aiBusy) return;
    setAiBusy(true);
    setError('');
    setMessage('');
    try {
      ...
      setDirty(true);
      setMessage('AI draft applied. Review it, then tap Save.');
      void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (caught) {
      setError(errorText(caught));
      void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 44 }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.safeCard}>
        ...
        <Text style={styles.safeText}>Mobile Save writes the same version-5 project schema. Publishing is intentionally not exposed here until the full validated publish/rollback pipeline is native-safe.</Text>
      </View>
      ...
      <View style={styles.createCard}>
        ...
        <Pressable disabled={!newProjectName.trim() || busy} onPress={() => void createProject()} style={[styles.primaryButton, (!newProjectName.trim() || busy) && styles.disabled]}>
      ...
      <View style={styles.headerRow}>
        ...
        <Pressable disabled={busy} onPress={() => void loadProjects(selectedId)} style={styles.iconButton}><MaterialCommunityIcons name="refresh" size={19} color={colors.text} /></Pressable>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.projectStrip}>
        {projects.map((row) => (
          <Pressable key={row.id} onPress={() => selectProject(row)} style={[styles.projectCard, selectedId === row.id && styles.projectActive]}>
            ...
            <Text style={styles.projectMeta}>{row.status === 'completed' ? 'Published state' : 'Draft'} · {shortDate(row.updated_at)}</Text>
          </Pressable>
        ))}
      </ScrollView>
      ...
      {draft ? <>
        ...
        <View style={styles.aiCard}>
          ...
          <Pressable disabled={!brief.trim() || aiBusy} onPress={() => void runAI()} style={[styles.aiButton, (!brief.trim() || aiBusy) && styles.disabled]}>{aiBusy ? <ActivityIndicator color={colors.white} /> : <MaterialCommunityIcons name="creation" size={20} color={colors.white} />}<Text style={styles.primaryText}>{aiBusy ? 'Building draft…' : 'Generate mobile draft'}</Text></Pressable>
        </View>
        ...
        {activePage ? <>
          ...
          <View style={styles.sectionList}>{activePage.sections.map((section, index) => <View key={section.id} style={styles.sectionCard}>
            <View style={styles.sectionCardHeader}><View style={styles.sectionNumber}><Text style={styles.sectionNumberText}>{index + 1}</Text></View><View style={{ flex: 1 }}><Text style={styles.sectionType}>{section.type.toUpperCase()}</Text><Text numberOfLines={1} style={styles.sectionTitle}>{section.title || 'Untitled section'}</Text></View>{activePage.sections.length > 1 ? <Pressable onPress={() => confirmRemoveSection(section)} style={styles.iconButton}><MaterialCommunityIcons name="trash-can-outline" size={18} color={colors.danger} /></Pressable> : null}</View>
            <Text style={styles.label}>Heading</Text><TextInput value={section.title} onChangeText={(value) => updateSection(section, 'title', value)} style={styles.input} maxLength={180} />
            <Text style={styles.label}>Description</Text><TextInput value={section.description} onChangeText={(value) => updateSection(section, 'description', value)} style={[styles.input, styles.multiline]} multiline textAlignVertical="top" maxLength={2000} />
            <View style={styles.twoCol}><View style={{ flex: 1 }}><Text style={styles.label}>Button text</Text><TextInput value={section.buttonText} onChangeText={(value) => updateSection(section, 'buttonText', value)} style={styles.input} maxLength={100} /></View><View style={{ flex: 1 }}><Text style={styles.label}>Button link</Text><TextInput value={section.buttonUrl} onChangeText={(value) => updateSection(section, 'buttonUrl', value)} style={styles.input} autoCapitalize="none" maxLength={1000} /></View></View>
        ...
        <View style={styles.actionsCard}>
          <Pressable disabled={!dirty || busy} onPress={() => void saveProject()} style={[styles.primaryButton, (!dirty || busy) && styles.disabled]}>{busy ? <ActivityIndicator color={colors.white} /> : <MaterialCommunityIcons name="cloud-upload-outline" size={20} color={colors.white} />}<Text style={styles.primaryText}>{busy ? 'Saving…' : 'Save to Tayar Cloud'}</Text></Pressable>
          {draft.content.publishedUrl ? <Pressable onPress={() => void openPublishedSite()} style={styles.secondaryButton}><MaterialCommunityIcons name="open-in-new" size={19} color="#DDD6FE" /><Text style={styles.secondaryText}>Open published site</Text></Pressable> : <View style={styles.publishNote}><MaterialCommunityIcons name="information-outline" size={18} color={colors.muted} /><Text style={styles.publishNoteText}>Publish will be added after the existing file validation + rollback pipeline is ported safely to mobile.</Text></View>}
          <Pressable disabled={busy} onPress={confirmArchive} style={styles.dangerButton}><MaterialCommunityIcons name="archive-outline" size={19} color={colors.danger} /><Text style={styles.dangerText}>Archive project</Text></Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  actionsCard: { marginTop: 16, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panel, padding: 14 }, secondaryButton: { minHeight: 46, borderRadius: radius.md, borderWidth: 1, borderColor: '#4A356F', backgroundColor: colors.violetSoft, marginTop: 9, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, secondaryText: { color: '#DDD6FE', fontSize: 12, fontWeight: '900' }, publishNote: { flexDirection: 'row', gap: 8, marginTop: 10, borderRadius: radius.md, backgroundColor: colors.panelSoft, padding: 11 }, publishNoteText: { flex: 1, color: colors.muted, fontSize: 10.5, lineHeight: 16 }, dangerButton: { minHeight: 44, borderRadius: radius.md, borderWidth: 1, borderColor: '#4B2430', backgroundColor: '#1E1116', marginTop: 9, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, dangerText: { color: colors.danger, fontSize: 12, fontWeight: '900' },
