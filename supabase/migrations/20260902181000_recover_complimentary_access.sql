-- Restore administrator-granted complimentary product access without mutating Stripe billing.
-- This migration intentionally layers on top of the recovered account controls and
-- subscription grace-period logic from 20260902154500.
...
BEGIN
  ...
  VALUES (
    'info',
    'admin',
    'Administrator changed complimentary product access',
    jsonb_build_object(
      'actor_id', auth.uid(),
...
-- Central entitlement calculation: suspension wins, then administrator access,
-- then a live complimentary grant, then the real Stripe subscription (including
-- the existing three-day past-due grace period). No complimentary grant writes to
-- or impersonates a paid subscription.
...
-- Keep the admin user list aligned with product entitlement calculation while
-- preserving a distinct admin label so administrator access is never mistaken for
-- Stripe revenue.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      ELSE public.team_effective_plan(p.id)
    END::text,
    p.role,
